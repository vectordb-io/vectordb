...
VIndexTransparent::VIndexTransparent(const std::string &path, VEngine* vengine, TransparentParam *param)
...
VIndexTransparent::VIndexTransparent(const std::string &path, VEngine* vengine)
    :vengine_(vengine),
     db_key2id_(nullptr),
     db_id2key_(nullptr),
     db_meta_(nullptr),
...
VIndexTransparent::~VIndexTransparent() {
    delete db_key2id_;
    delete db_id2key_;
    delete db_meta_;
    delete annoy_index_;
...
Status
VIndexTransparent::GetKNN(const std::string &key, int limit, std::vector<VecDt> &results) {
    if (limit > 0) {
        VecObj vo;
        auto s = vengine_->Get(key, vo);
        if (!s.ok()) {
...
Status
VIndexTransparent::GetKNN(const std::vector<float> &vec, int limit, std::vector<VecDt> &results) {
    if (static_cast<int>(vec.size()) != dim_) {
        return Status::OtherError("dim not equal");
    }

    if (limit > 0) {
        std::vector<VecDt> results_out;
        ...
        for (it->SeekToFirst(); it->Valid(); it->Next()) {
            ...
            float distance;
            auto s = Distance(vec, arr, distance_type_, distance);
            if (!s.ok()) {
                return s;
            }
...
Status
VIndexTransparent::ProcResults(const std::vector<std::string> results, const std::vector<float> distances, std::vector<VecDt> &results_out) {
    assert(results.size() == distances.size());
    results_out.clear();

    for (size_t i = 0; i < results.size(); ++i) {
        std::string find_key = results[i];
...
/*
Status
VIndexTransparent::Distance(const std::string &key1, const std::string &key2, float &distance) {
    int id1, id2;
...
Status
VIndexTransparent::CheckParams() const {
    if (index_type_ != VINDEX_TYPE_TRANSPARENT) {
...
Status
VIndexTransparent::Key2Id(const std::string &key, int &id) const {
    std::string value;
    leveldb::Status ls;
...
Status
VIndexTransparent::Id2Key(int id, std::string &key) const {
    std::string id_string;
    leveldb::Status ls;
...
jsonxx::json64
VIndexTransparent::ToJson() const {
    ...
    jret["VIndexTransparent"] = j;
    return jret;
}

std::string
VIndexTransparent::ToString() const {
    return ToJson().dump();
}

std::string
VIndexTransparent::ToStringPretty() const {
    return ToJson().dump(4, ' ');
}

Status
VIndexTransparent::PersistMeta() {
...
float
VIndexTransparent::Dt2Cos(float dt) {
    return 1 - dt / 2;
}

Status
VIndexTransparent::Distance(const std::vector<float> &vec1, const std::vector<float> &vec2, const std::string &distance_type, float &distance) {
