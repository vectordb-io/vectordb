...
void CliConfig::Parse(int32_t argc, char **argv) {
  ...
  options_->add_options()(
      ...
      "path", "home path", cxxopts::value<std::string>()->default_value(""))(
...
const std::string CliConfig::Usage() {
  std::string str;
  if (options_) {
    str = options_->help();
  }
...
const std::string CliConfig::UsageBanner(char *program_name) {
  std::string str = "Example:\n";
  char buf[256];
  snprintf(buf, sizeof(buf), "%s --addr=127.0.0.1:9000 --debug\n",
           program_name);
  str.append(buf);
  snprintf(buf, sizeof(buf), "%s --path=/tmp/local_console --debug\n",
           program_name);
  str.append(buf);
  snprintf(buf, sizeof(buf), "%s --path=/tmp/local_console\n", program_name);
  str.append(buf);
  return str;
}

const std::string CliConfig::ProgramName() { return options_->program(); }

const std::string CliConfig::ToString() const {
  std::string str;
  char buf[512];
...
std::mutex CliConfigSingleton::mu_;
CliConfigSPtr CliConfigSingleton::instance_ = nullptr;
