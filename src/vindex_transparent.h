#ifndef __VECTORDB_VINDEX_TRANSPARENT_H__
#define __VECTORDB_VINDEX_TRANSPARENT_H__
  ...
  public:
    ...
    Create(const std::string &distance_type, int dim) {
        ...
        if (distance_type == VINDEX_DISTANCE_TYPE_COSINE) {
            ...
            std::string msg = "create transparent index error, unknown distance_type ";
...
class VIndexTransparent : public VIndex {
  public:
#define KEY_META_TRANSPARENT_INDEX "KEY_META_TRANSPARENT_INDEX"
  ...
  public:
    VIndexTransparent(const std::string &path, VEngine* vengine, TransparentParam *param);    // call Build, path: /tmp/table/partition/replica/index/  will create dir table#annoy#xxx
    VIndexTransparent(const std::string &path, VEngine* vengine);                       // call Load,  path: /tmp/table/partition/replica/index/table#annoy#xxx
    VIndexTransparent(const VIndexTransparent&) = delete;
    VIndexTransparent& operator=(const VIndexTransparent&) = delete;
    ~VIndexTransparent();
  ...
  private:
    VIndexParam PrepareVIndexParam(const std::string &path, TransparentParam *param) {
    ...
    Status LoadMeta();
    Status LoadAnnoy();
    Status ProcResults(const std::vector<std::string> results, const std::vector<float> distances, std::vector<VecDt> &results_out);
