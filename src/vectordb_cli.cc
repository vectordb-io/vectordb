...
VectordbCli::Do(const std::vector<std::string> &cmd_sv, const std::string &params_json) {
    ...
    if (cmd_sv.size() == 0) {
        ...
        auto s = PreProccessCreateTable(params_json, request, reply);
        if (s.ok()) {
        ...
        auto s = PreProccessPing(params_json, request, reply);
        if (s.ok()) {
            Ping(request, reply);
        }
...
Status
VectordbCli::PreProccessCreateTable(const std::string &params_json, vectordb_rpc::CreateTableRequest &request, std::string &reply_msg) {
    reply_msg.clear();
    do {
        std::string table_name;
        int partition_num;
...
Status
VectordbCli::PreProccessPing(const std::string &params_json, vectordb_rpc::PingRequest &request, std::string &reply_msg) {
    reply_msg.clear();
