...
void RemuTick(vraft::Timer *timer) {
  switch (vraft::current_state) {
    ...
    case vraft::kTestState9: {
      ...
      vraft::current_state = vraft::kTestState10;

      break;
    }

    // check sm consistant
    case vraft::kTestState10: {
      vraft::TestSM *psm =
          (vraft::TestSM
...
void PrintDesc() {
  if (vraft::gtest_desc) {
    ...
    snprintf(buf, sizeof(buf), "step%d: stop leader \n", step++);
    desc.append(buf);

    snprintf(buf, sizeof(buf), "step%d: wait until elect leader \n", step++);
    desc.append(buf);

    snprintf(buf, sizeof(buf), "step%d: propose 5 values \n", step++);
    desc.append(buf);

    snprintf(buf, sizeof(buf), "step%d: start the old leader \n", step++);
    desc.append(buf);
